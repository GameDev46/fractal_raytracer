...
float mountainSDF(vec2 p) {

	vec2 pos = vec2(p.x, p.y);

  pos *= 0.01;

	float height = sineFBM(pos * 0.5) * 0.5;
	height += sineFBM(pos + vec2(0.2)) * 0.5;

	height *= sineFBM(pos + vec2(0.2) + vec2(fractalFold * 2.0)) * 0.3 * fractalFloat * 7.49063670412;
...
float terrainSDF(vec3 p, vec3 pos, inout GameObject terrain) {

	float height = mountainSDF(vec2(p.x, p.z));

	vec3 sand = vec3(0.89, 0.88, 0.57);
	vec3 grass1 = terrain.colour;
	vec3 grass2 = vec3(0.32, 0.38, 0.09);
	vec3 grass3 = vec3(0.38, 0.45, 0.15);
	vec3 dirt1 = vec3(0.58, 0.54, 0.48);
	vec3 dirt2 = vec3(0.50, 0.48, 0.44);
	vec3 dirt3 = vec3(0.32, 0.31, 0.28);

	// Sand
	terrain.colour = mix(sand, grass1, min(1.0, max(0.0, min(1.0, (height - 0.48) * 30.0))));
	// Grass layer 1
	terrain.colour = mix(terrain.colour, grass2, min(1.0, max(0.0, min(1.0, (height - 0.52) * 30.0))));
	// Grass layer 2
	terrain.colour = mix(terrain.colour, grass3, min(1.0, max(0.0, min(1.0, (height - 0.58) * 30.0))));
	// Grass layer 3
	terrain.colour = mix(terrain.colour, dirt1, min(1.0, max(0.0, min(1.0, (height - 0.64) * 30.0))));

	// Dirt layer 1
	terrain.colour = mix(terrain.colour, dirt2, min(1.0, max(0.0, min(1.0, (height - 0.7) * 30.0))));
	// Dirt layer 2 + 3
	terrain.colour = mix(terrain.colour, dirt3, min(1.0, max(0.0, min(1.0, (height - 0.76) * 30.0))));

	terrain.colour *= 0.5;

	return (planeSDF(p, pos, vec3(0, 1, 0)) - height * 30.0) + 30.0;
}
...
GameObject getSceneSDF(vec3 p) {
	...
	GameObject land = GameObject(vec3(0.0, 0.0, 8.0), vec3(0.37, 0.45, 0.13), vec3(2.0, 2.0, 2.0), 0.0, 0.0, 0.0);
	land.distance = terrainSDF(p, vec3(-5.0), land) - 18.0;
	...
	if (land.distance < water.distance) {
		return land;
	}
