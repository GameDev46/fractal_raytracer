...
vec3 rotateYAxis(vec3 p, float angle) {
  float c = cos(angle);
  float s = sin(angle);
...
float fractalTreeSDF(vec3 p, inout GameObject fractal) {

	p *= 1.0 / fractalScale;

	//float sphere = sphereSDF(p, vec3(0.0), vec3(1.875));
	
	float sphere = torusSDF(p, vec3(0.0), vec3(1.875, 1.4, 0.0));
	float scale = fractalFloat * 7.5;

	float transformations = 0.0;
	const int iterations = 8;

	for (int n = 0; n < iterations; n++) {

		float prevDist = sphere;

		boxFold(p, fractalFold);
		sphereFold(p, fractalFold);

		float dist = torusSDF(mod(p, scale * 2.0), vec3(scale), vec3(scale * 0.5, scale * 0.4, 0.0));

		sphere = max(-dist, sphere);

		if (prevDist != sphere) {
			transformations = float(n);
		}

		scale *= fractalRotate * 0.5;

	}

	float normTrans = (transformations / float(iterations));

	fractal.colour = vec3( normTrans * 0.7, 0.3, (1.0 - normTrans) * 1.0 );
...
GameObject getSceneSDF(vec3 p) {
	...
	GameObject fractal = GameObject(vec3(0.0, 0.0, 8.0), vec3(0.698, 0.9745, 0.533), vec3(2.0, 2.0, 2.0), 0.0, 0.0, 0.0);
	fractal.distance = fractalTreeSDF(p, fractal);
