...
float mandlebulbSDF(vec3 pos, inout GameObject fractal) {

	pos *= 1.0 / fractalScale;

	vec3 z = pos;
	float dr = 1.0 / fractalScale;
	float r = 0.0;

	float power = fractalFloat * 22.4719101;
	...
	fractal.colour = vec3(0.21, iterations * 0.7, (1.0 - iterations) * 0.49);
...
GameObject getSceneSDF(vec3 p) {
	...
	GameObject mandleBulb = GameObject(vec3(0.0, 0.0, 8.0), vec3(0.698, 0.9745, 0.533), vec3(2.0, 2.0, 2.0), 0.0, 0.0, 0.0);
	mandleBulb.distance = mandlebulbSDF(p, mandleBulb);
