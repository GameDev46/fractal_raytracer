...
float sponge(vec3 p, inout GameObject fractal) {

   float d = cubeSDF(p, vec3(0.0), vec3(fractalScale * 1.875));
	 float savedD = d;

   float s = (fractalFloat * 7.5) / (fractalScale * 1.875);
	 float mFloat = 4.0;
   ...
   for(int m = 0; m < 6; m++)
   {
			p.xy = foldRotate(p.xy, fractalRotate);
			p.yz = foldRotate(p.yz, fractalRotate);
	 ...
	 fractal.colour = vec3(0.5, 0.0, 1.0);
...
GameObject getSceneSDF(vec3 p) {
	...
	fractal.distance = sponge(p, fractal);
