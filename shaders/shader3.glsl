...
float sineFBM(vec2 p) {
	...
	for (int i = 0; i < 8; i++) {
		...
		res += amp * sineNoise(p);
		amp *= 0.5;
		p = p * freq * rot(3.14159265 / 4.0) - res * 0.4;
...
GameObject getSceneSDF(vec3 p) {
	...
	vec3 worldOffset = vec3(0.0, -scale, 0.0);

	GameObject floor = GameObject(vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0), vec3(2.0, 2.0, 2.0), 0.0, 0.0, 0.0);
	floor.distance = cubeSDF(p, floor.position + worldOffset, vec3(scale, scale * 0.015, scale) );

	GameObject wall1 = GameObject(vec3(-scale, scale, 0.0), vec3(0.698, 0.9745, 0.533), vec3(2.0, 2.0, 2.0), 0.0, 0.0, 0.0);
	wall1.distance = cubeSDF(p, wall1.position + worldOffset, vec3(scale * 0.015, scale, scale) );

	GameObject backWall = GameObject(vec3(0.0, scale, -scale * 2.0), vec3(0.698, 0.533, 0.9745), vec3(2.0, 2.0, 2.0), 0.0, 0.0, 0.0);
	backWall.distance = cubeSDF(p, backWall.position + worldOffset, vec3(scale, scale, scale * 0.015) );

	GameObject wall2 = GameObject(vec3(scale, scale, 0.0), vec3(0.9745, 0.533, 0.698), vec3(2.0, 2.0, 2.0), 0.0, 0.0, 0.0);
	wall2.distance = cubeSDF(p, wall2.position + worldOffset, vec3(scale * 0.015, scale, scale) );

	float closestObjDistance = min( min( min( wall1.distance, backWall.distance ), floor.distance ), wall2.distance );

	GameObject objectToReturn = floor;

	if (closestObjDistance == wall1.distance) objectToReturn = wall1;
	if (closestObjDistance == backWall.distance) objectToReturn = backWall;
	if (closestObjDistance == wall2.distance) objectToReturn = wall2;
	...
	for (int i = 0; i < layers; i++) {
		...
		float reflectivity = float(i) / (float(layers) - 1.0);
		float offset = float(i) / (float(layers) - 1.0);

		GameObject cube = GameObject(vec3(-scale * 0.5, scale * 0.2, -scale * (0.25 + offset)), vec3(0.9745, 0.533, 0.698), vec3(2.0, 2.0, 2.0), 0.0, reflectivity, sign(float(i)));
		cube.distance = cubeSDF(p, cube.position + worldOffset, vec3(scale * 0.1) );

		if (i > 0) {
			cube.colour = vec3(5.0);
		}

		if (cube.distance < closestObjDistance) {
			objectToReturn = cube;
			closestObjDistance = cube.distance;
		}

		GameObject sphere = GameObject(vec3(0.0, scale * 0.2, -scale * (0.4 + offset)), vec3(0.9745, 0.233, 0.9745), vec3(2.0, 2.0, 2.0), 0.0, reflectivity, sign(float(i)));
		sphere.distance = sphereSDF(p, sphere.position + worldOffset, vec3(scale * 0.1) );

		if (i > 0) {
			sphere.colour = vec3(5.0);
		}

		if (sphere.distance < closestObjDistance) {
			objectToReturn = sphere;
			closestObjDistance = sphere.distance;
		}

		GameObject torus = GameObject(vec3(scale * 0.5, scale * 0.2, -scale * (0.4 + offset)), vec3(0.233, 0.833, 0.9745), vec3(2.0, 2.0, 2.0), 0.0, reflectivity, sign(float(i)));
		torus.distance = torusSDF(p, torus.position + worldOffset, vec3(scale * 0.1, scale * 0.025, 0.0) );

		if (i > 0) {
			torus.colour = vec3(5.0);
		}

		if (torus.distance < closestObjDistance) {
			objectToReturn = torus;
			closestObjDistance = torus.distance;
		}
