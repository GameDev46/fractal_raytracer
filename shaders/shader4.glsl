...
float randomFractal(vec3 p, inout GameObject fractal) {
	...
	for (int n = 0; n < iterations; n++) {
		...
		float dist = sphereSDF(mod(p, scale * 2.0), vec3(scale), vec3(scale));
	...
	fractal.colour = vec3( normTrans * 1.0, 0.5, (1.0 - normTrans) * 0.7 );
...
GameObject getSceneSDF(vec3 p) {
	...
	fractal.distance = randomFractal(p, fractal);
